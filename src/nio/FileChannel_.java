...
public class FileChannel_ {
    ...
    private void copyWay1() throws IOException {//利用IO流通过管道完成文件的复制
        FileInputStream fis = new FileInputStream("D:\\cosmos\\test\\fis.txt");
        ...
        //实际使用需要用try-catch-finally
        outChannel.close();
        inChannel.close();
        fos.close();
        fis.close();
    }
    private void copyWay2() throws IOException {//使用直接缓冲区完成文件复制(速度更快,但不一定是最好)
        //在JDK1.7中的NIO2,针对通道提供了静态方法open()
    ...
    private void copyWay3() throws IOException {//使用transfer方法完成通道之间的数据传输(和方法2一样,不过是更方便)
        FileChannel inChannel = FileChannel.open(Paths.get("D:\\cosmos\\test\\in.txt"), StandardOpenOption.READ);
        FileChannel outChannel = FileChannel.open(Paths.get("D:\\cosmos\\test\\out.txt"), StandardOpenOption.READ,StandardOpenOption.WRITE,StandardOpenOption.CREATE);//CREATE没有则创建,有则覆盖;CREATE_NEW没有则创建,有则报错
