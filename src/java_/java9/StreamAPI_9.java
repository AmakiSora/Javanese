...
public class StreamAPI_9 {
    ...
    private void optional_Stream() {
        ArrayList<String> list = new ArrayList<>();
        list.add("a");
        list.add("b");
        list.add("c");
        Optional<ArrayList<String>> optional = Optional.ofNullable(list);
        Stream<ArrayList<String>> stream = optional.stream();//容器转换成流
        stream.flatMap(t -> t.stream()).forEach(System.out::println);
        System.out.println("----------");
    }
    public void test() {
        takeWhile_();
